...
function BreakdownBar({ label, value, color }) {
  return (
    <div style={{ background:"#131920", border:"1px solid #1f2937", borderRadius:"10px", padding:"16px" }}>
      <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"10px" }}>
        {label}
      </p>
      <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
        <div style={{ flex:1, height:"6px", background:"#1f2937", borderRadius:"3px", overflow:"hidden" }}>
          <div style={{ height:"100%", width:`${value*100}%`, background:color, borderRadius:"3px", transition:"width 0.5s ease" }}/>
        </div>
...
export default function IPODetail() {
  ...
  if (loading) return (
    <div style={{ padding:"80px", textAlign:"center", color:"#475569" }}>Loading...</div>
  );

  if (error || !d || d.error) return (
    <div style={{ padding:"80px", textAlign:"center", color:"#ef4444" }}>IPO not found.</div>
  );
  ...
  return (
    <div style={{ padding:"40px 60px", maxWidth:"1200px", margin:"0 auto", display:"flex", flexDirection:"column", gap:"28px" }}>

      {/* Back */}
      <button onClick={() => navigate("/")} style={{
        display:"inline-flex", alignItems:"center", gap:"6px",
        background:"none", border:"none", color:"#475569", cursor:"pointer",
        fontSize:"13px", fontFamily:"'JetBrains Mono',monospace", padding:0,
      }}>
        ← Back to Dashboard
      </button>
      ...
      <div style={{ borderBottom:"1px solid #1f2937", paddingBottom:"24px" }}>
        <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"8px" }}>
          IPO Analysis
        </p>
        <h1 style={{ fontSize:"32px", fontWeight:800, fontFamily:"Manrope,sans-serif", color:"#f1f5f9", marginBottom:"14px" }}>
          {d.ipo_name}
        </h1>
        <div style={{ display:"flex", gap:"10px", alignItems:"center" }}>
          ...
          <span style={{
            ...
            background: confColor(d.confidence) + "20",
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"16px" }}>
        <div style={cardStyle}>
          <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"8px" }}>Final Score</p>
          <p style={{ fontSize:"32px", fontWeight:800, fontFamily:"Manrope,sans-serif", color: sigColor(d.signal) }}>{d.final_score?.toFixed(4)}</p>
        </div>
        <div style={cardStyle}>
          <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"8px" }}>Articles</p>
          <p style={{ fontSize:"32px", fontWeight:800, fontFamily:"Manrope,sans-serif", color:"#eab308" }}>{d.article_count}</p>
        </div>
        <div style={cardStyle}>
          <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"8px" }}>Avg Sentiment</p>
          <p style={{ fontSize:"32px", fontWeight:800, fontFamily:"Manrope,sans-serif", color: sentColor(d.avg_sentiment_score) }}>
      ...
      {/* Gauge */}
      <div style={cardStyle}>
        <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"16px" }}>Signal Gauge</p>
      ...
      {/* Score breakdown */}
      <div>
        <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"14px" }}>Score Breakdown</p>
      ...
      {d.trend && d.trend.length > 1 && (
        <div style={cardStyle}>
          <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"16px" }}>Sentiment Trend</p>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={d.trend}>
              <XAxis dataKey="week" tick={{ fill:"#475569", fontSize:10 }} axisLine={false} tickLine={false}/>
              <YAxis tick={{ fill:"#475569", fontSize:10 }} axisLine={false} tickLine={false}/>
              <Tooltip
                contentStyle={{ background:"#0d1117", border:"1px solid #1f2937", borderRadius:"8px", fontSize:"12px" }}
                labelStyle={{ color:"#94a3b8" }}
                itemStyle={{ color: sigColor(d.signal) }}
              />
      ...
      <div style={{ ...cardStyle, background:"#0d1117" }}>
        <p style={{ color:"#475569", fontSize:"10px", fontWeight:800, letterSpacing:"1.5px", textTransform:"uppercase", fontFamily:"'JetBrains Mono',monospace", marginBottom:"12px" }}>
          Coming Soon
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:"12px" }}>
          {["📊 GMP (Grey Market Premium)","📋 Promoter Holdings","🔄 Subscription Status","💰 OFS Details","📈 Price Band & Lot Size","🏛️ Anchor Investors"].map(item => (
